...
class ShuffleNetV2(BaseBackbone):
    def __init__(self,
        ...
        self.layers.append(
            ConvModule(
                ...
                act_cfg=act_cfg))
        if args != None:
            if args.kd_type == 'norm':
                if 'clip' in args.foundation_model_name_or_path:
                    teacher_out_dim = 2048
                elif 'rad' in args.foundation_model_name_or_path:
                    teacher_out_dim = 1024
                elif 'lvm' in args.foundation_model_name_or_path:
                    teacher_out_dim = 768
                self.layer31 = conv_1x1_bn(self.out_dim, teacher_out_dim * 8)
                self.layer32 = conv_1x1_bn(teacher_out_dim * 8, self.out_dim)
    ...
    def forward(self, x, return_intermediate_features=False, norm_flag=False):
        ...
        if norm_flag: # only for train
            res = outs[-1]
            x5 = self.layer31(outs[-1])  # 16x16
            f3 = x5
            x5 = self.layer32(x5)  # 16x16
            x5 += res

            if return_intermediate_features:
                return [x5, f3], None, None
            else:
                return [x5, f3]

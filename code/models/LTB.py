        ...
        for i in range(5): # i: layer idx
            for tn, task in enumerate(self.task_name): # tn: task idx
                if i == 0:
                    ...
                    child_rep = sum([alpha[i,tn,j].to(ss_rep[i-1][j].device)*ss_rep[i-1][j] for j in range(self.task_num)]) # j: module idx
...
class LTB(AbsArchitecture):
    ...
    def __init__(self, task_name, encoder_class, rep_grad, multi_input, device, **kwargs):
        super(LTB, self).__init__(task_name, encoder_class, rep_grad, multi_input, device, **kwargs)

    ...
    def step_with_model(self, data: torch.Tensor, targets: dict, model: torch.nn.Module, criteria: dict, **kwargs):
        def loss_fn(task_id):
            if isinstance(model, torch.nn.DataParallel):
                ...
                task_loss = criteria[task_id](model.module.decoders[task_id](hrepr), targets[task_id])
                return precision * task_loss + 0.5 * model.module.decoders[task_id].log_var
            else:
                precision = (-model.decoders[task_id].log_var).exp()
        ...
        if self.compute_stats:
            if isinstance(model, torch.nn.DataParallel):
                G = self.get_G_wrt_shared(losses, list(model.module.encoder.parameters()))
            else:

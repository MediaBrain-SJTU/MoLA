...
class NYUv2Evaluator:
    ...
    def _compute_stats_task(model, data_loader, device):
        ...
        for data in data_loader:
            ...
            # depths
            task_th = 1
            hrepr = model.encoder(input.to(device), task_label=task_th)
    ...
    def _compute_stats_each_task(model, data_loader, device):
        ...
        for data in data_loader:
            ...
            p_depth = model.decoders["DE"](hrepr[task_th]).to(gt_depth.device)
            arr_pred_depth.append(p_depth)
            arr_gt_depth.append(gt_depth)
            ...
            p_normal = model.decoders["NE"](hrepr[task_th]).cpu().to(gt_depth.device)
            arr_pred_normal.append(p_normal)
            arr_gt_normal.append(gt_normal)
            ...
            conf_mat.update(
                model.decoders["SS"](hrepr[task_th]).argmax(1).flatten().cpu(),
                gt_semantic.flatten(),
            )
            pbar.update(1)
    ...
    def evaluate_task(model, data_loader, device):
        ...
        with torch.no_grad():
            (
                ...
                normal_errs,
                depth_errs,
            ) = NYUv2Evaluator._compute_stats_task(model, data_loader, device)
        print(
            f"mIOU: {m_segm_miou}, Pix Acc: {m_segm_pix_acc}; "
            f"Abs.: {depth_errs[0]}, Rel.: {depth_errs[1]};"
            f" Angle Dist (mean): {normal_errs[0]}, Angle Dist (median): {normal_errs[1]},"
    ...
    def evaluate_each_task(model, data_loader, device):
        ...
        with torch.no_grad():
            (
                ...
                normal_errs,
                depth_errs,
            ) = NYUv2Evaluator._compute_stats_each_task(model, data_loader, device)
        print(
            f"mIOU: {m_segm_miou}, Pix Acc: {m_segm_pix_acc}; "
            f"Abs.: {depth_errs[0]}, Rel.: {depth_errs[1]};"
            f" Angle Dist (mean): {normal_errs[0]}, Angle Dist (median): {normal_errs[1]},"

...
class MultiLoraConv2dTask(LoRALayer):
    def __init__(self, r, lora_alpha, conv_layer, num_task):
        """
        LoRA class for nn.Conv2d class
        """
    ...
    def forward(self, input_x, alphas=None):
        ...
        batch_size, c = x.shape[0], x.shape[1]
        self.conv.weight.data = self.conv.weight.data + (self.lora_B_list[alphas] @ self.lora_A_list[alphas]).view(self.conv.weight.shape)* self.scaling
...
class MultiLoraConv2dV3(LoRALayer):
    def __init__(self, r, lora_alpha, conv_layer, num_task):
        """
        LoRA class for nn.Conv2d class
        """

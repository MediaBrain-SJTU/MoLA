...
class MultiLoraConv2d(LoRALayer):
    ...
    def forward(self, x):
        if not self.merged:
            if self.label_batch is not None:
                label_batch = self.label_batch
            else:
                label_batch = list(range(0,self.num_task))

            multi_b = x.shape[0]
            interval = multi_b//(len(label_batch)+1)
            outputs = F.conv2d(x[:interval], self.conv.weight, self.conv.bias, self.conv.stride, self.conv.padding, self.conv.dilation, self.conv.groups)
            multi_expert_outputs = [F.conv2d(x[interval*(th+1):interval*(th+2)], self.conv.weight + (self.lora_B_list[th] @ self.lora_A_list[th]).view(self.conv.weight.shape) * self.scaling, self.conv.bias, self.conv.stride, self.conv.padding, self.conv.dilation, self.conv.groups) for th in range(self.num_task) if th in label_batch]
            for th in range(self.num_task):
                if th in label_batch:
                    outputs = torch.vstack((outputs, multi_expert_outputs[th]))
            assert outputs.shape[0] == multi_b, print(outputs.shape[0], multi_b)
            return outputs
        else:
            return self.conv(x)
